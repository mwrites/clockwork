...
use mat_clockwork_network_program::state::{Fee, Pool, Worker, WorkerAccount};
use mat_clockwork_utils::thread::{SerializableInstruction, ThreadResponse, PAYER_PUBKEY};
...
pub struct ThreadExec<'info> {
    ...
    #[account(
        mut,
        seeds = [
            mat_clockwork_network_program::state::SEED_FEE,
            worker.key().as_ref(),
        ],
        bump,
        seeds::program = mat_clockwork_network_program::ID,
        has_one = worker,
    )]
